...
describe('Criar uma conta', () => {
    describe('Criar conta utilizando um email de uma conta ativa', () => {
        ...
        it(`Quando o usuário insere um email já cadastrado "${input.emailAdress}" no campo "Email adress" em "Create an account"`, () => {
            loginForm.typeEmailAdress(input.emailAdress)
        })
        it(`E clica no botão "Create an account"`, () => {
            loginForm.clickSignIn()
        })
        it(`Então vê a mensagem "An account using this email adress has already been registered. Please enter a valid password or request a new one. "`, () => {
            loginForm.elements.authFeedback().should('contains.text', 'An account using this email adress has already been registered. Please enter a valid password or request a new one.')
        })
