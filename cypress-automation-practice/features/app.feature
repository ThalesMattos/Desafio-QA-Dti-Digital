...
Cenário: Login falho com senha inválida
    ...
    Quando o usuário insere um e-mail válido "email@valido.com" no campo "Email adress" em "Already registered?"
...
Cenário: Criar conta utilizando um email de uma conta ativa
    Dado que o usuário está na página de sign in
    Quando o usuário insere um email já cadastrado "emailativo@exemplo.com" no campo "Email adress" em "Create an account"
    E clica no botão "Create an account"
    Então vê a mensagem "An account using this email adress has already been registered. Please enter a valid password or request a new one. "
    E permanece na página de sign in
...
Cenário: Recuperar senha com email não cadastrado
    ...
    Então uma mensagem de erro é exibida "There is no account registered for this email adress."
